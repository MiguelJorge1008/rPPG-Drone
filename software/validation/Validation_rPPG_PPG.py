...
class ValidationPlotter:
    def __init__(self, port, baud, cam, imu=None, algo="GREEN", roi=ROI_FACE):
        ...
        self.cam = cam
        # Pass the camera, IMU, algo, and ROI to the processor
        ...
        try:
            while self.running:
                ...
                if frame_count % BUFFER_UPDATE == 0:
                    ...
                    # ---------------------------------------------------------
                    # 2. SOFTWARE UPDATE (Physiologically Honest)
                    # ---------------------------------------------------------
                    if self.current_rppg_bvp is not None:
                        ...
                        if fs > 0 and len(display_bvp) > 15:
                            # 1. Get the FFT BPM for display (we no longer use it to lock the signal)
                            fft_bpm = 70.0 
                            ...
                            # 2. The signal is already filtered to 0.75 - 3.33 Hz by Processor.py!
                            # We just standardize it to keep peak heights uniform for the peak finder.
                            bvp_std = np.std(display_bvp)
                            if bvp_std > 0:
                                display_bvp = (display_bvp - np.mean(display_bvp)) / bvp_std

                            # 3. Peak Prediction / Validation 
                            ...
                            if len(candidates) > 0 and fft_bpm > 40:
                                ...
                                for cand in candidates[1:]:
                                    time_since_last = cand - validated_peaks[-1]
