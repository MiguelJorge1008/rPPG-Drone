            ...
            while self.running:
                ...
                if frame_count % BUFFER_UPDATE == 0:
                    ...
                    if min_len > 50:
                        ...
                        # use same DISPLAY_SECS window for RPM (consistent with software)
                        hw_t_calc = hw_t_arr[mask]
                        hw_r_calc = hw_r_arr[mask]
                        hw_peaks, _ = find_peaks(hw_r_calc, distance=75, prominence=0.05)
                        if len(hw_peaks) >= 2:
                            bb_intervals = np.diff(hw_t_calc[hw_peaks])
