...
HR_WINDOW_S   = 30    # GT computation window (matches Processor.estimate_hr)
            ...
            while self.running:
                ...
                if frame_count % BUFFER_UPDATE == 0:
                    ...
                    if min_len > 100:
                        ...
                        # use same DISPLAY_SECS window for BPM
                        hw_t_calc = hw_t_arr[mask]
                        hw_p_calc = hw_p_arr[mask]

                        # HR via power FFT — identical to Processor.estimate_hr (no filtering)
                        # use last HR_WINDOW_S seconds for GT computation
                        t_cutoff_hr = hw_t_arr[-1] - HR_WINDOW_S
                        mask_hr = hw_t_arr >= t_cutoff_hr
                        hw_t_hr = hw_t_arr[mask_hr]
                        hw_p_hr = hw_p_arr[mask_hr]
                        dt_hw = np.median(np.diff(hw_t_hr)) if len(hw_t_hr) > 1 else 0
                        if dt_hw > 0 and len(hw_p_hr) >= 10:
                            fs_hw = 1.0 / dt_hw
                            freqs = np.fft.rfftfreq(len(hw_p_hr), d=1.0 / fs_hw)
                            power = np.abs(np.fft.rfft(hw_p_hr)) ** 2
                            band  = (freqs >= 0.75) & (freqs <= 4.0)
                            if band.any():
                                current_hw_bpm = freqs[band][np.argmax(power[band])] * 60.0

                        # SDNN / RMSSD via find_peaks (need beat intervals)
                        hw_peaks, _ = find_peaks(hw_p_calc, distance=int(0.4 * fs_hw) if dt_hw > 0 else 40,
                                                 prominence=0.05)
                        if len(hw_peaks) >= 3:
